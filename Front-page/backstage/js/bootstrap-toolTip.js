...
(function($,w){
    ...
    /**
     * @description 消息弹出框
     * 调用方法  $.toolTip({type,content,interval,success()})
     * @param options obj
     * @param type String 提示类型 success info warning danger
     * @param content String 提示的内容
     * @param interval 提示框消失的时间  单位为秒
     * @param success  {function} 倒计时结束后的执行的函数
     * @author guozhitao
     * 注：此方法在有jQuery和bootstrap的前提下使用
     * */ 
    $.toolTip = function(options){
        var alert = $(`
            <div class="modal fade modul-register" tabindex="-1" id="myModal" role="dialog">
